...
const logger = loggerService.withContext('OpenAIImagePage')

interface OpenAIImagePageProps {
  providerId: string
  Options: string[]
}

const OpenAIImagePage: FC<OpenAIImagePageProps> = ({ providerId, Options }) => {
  ...
  const filteredPaintings = useMemo(() => imagePaintings[mode] || [], [imagePaintings, mode])
  ...
  const modelOptions = useMemo(() => {
    ...
    const customModels = currentProvider.models
      .filter(
        (m) =>
          m.capabilities?.some((c) => c.type === 'image' && c.isUserSelected !== false) ||
          isGenerateImageModel(m)
      )
      .map((m) => ({
        label: m.name,
        value: m.id,
  ...
  const onGenerate = async () => {
    ...
    const url = currentProvider.apiHost + `/v1/images/generations`
    const editUrl = currentProvider.apiHost + `/v1/images/edits`
  ...
  const handleModeChange = (value: string) => {
    ...
    if (imagePaintings[value as keyof PaintingsState] && imagePaintings[value as keyof PaintingsState].length > 0) {
      setPainting(imagePaintings[value as keyof PaintingsState][0])
    } else {
    ...
    <Container>
      ...
      <ContentContainer id="content-container">
        <LeftContainer>
          ...
          <Select
            value={providerOptions.find((p) => p.value === providerId)?.value}
...
export default OpenAIImagePage
