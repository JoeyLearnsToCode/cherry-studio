...
import { continueGenerationThunk,sendMessage as _sendMessage } from '@renderer/store/thunk/messageThunk'
import {
  type Assistant,
  type FileMetadata,
  type KnowledgeBase,
...
const InputbarInner: FC<InputbarInnerProps> = ({ assistant: initialAssistant, setActiveTopic, topic, actionsRef }) => {
  ...
  const sendMessage = useCallback(async () => {
    const inputEmpty = text.length === 0 && files.length === 0
    const inputBlank = text.trim().length === 0 && files.length === 0
    const isLastMessageAssistant = messages.length > 0 && messages[messages.length - 1].role === 'assistant'
    const isWhitespaceOnly = text.length > 0 && text.trim().length === 0 && files.length === 0
    ...
    try {
      ...
      const alreadyUploadedFiles = files.filter((f) => (f as any)._alreadyUploaded)
      const localFiles = files.filter((f) => !(f as any)._alreadyUploaded)
