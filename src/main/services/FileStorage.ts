...
class FileStorage {
  ...
  public downloadImageToLocal = async (_: Electron.IpcMainInvokeEvent, url: string): Promise<FileMetadata> => {
    try {
  ...
  public saveBase64ImageToLocal = async (_: Electron.IpcMainInvokeEvent, base64Data: string): Promise<FileMetadata> => {
    try {
      if (!base64Data) {
        throw new Error('Base64 data is required')
      }
